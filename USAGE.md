...
- **Signal stops**: When a train reaches a main signal (SIGNAL_M3 or SIGNAL_COMBINED) at aspect 0 (red), it stops and waits. Signals only block trains approaching from the front (the direction the signal faces based on rotation). Trains approaching a signal from behind ignore it. Before the signal turns green, the block ahead is reserved for this train. If that block is already reserved by another train, or if any gap tile between the signal and the block is reserved by another simulation, the signal stays red. **Signal reset to red**: When a green signal is passed by the train head (5 tiles ahead by default, configurable via `setSignalResetDistance`), it automatically resets to red.
...
- **Gap tile reservation**: Turnout tiles between blocks ("gap tiles") are reserved along with the block. When two trains share overlapping routes, each simulation checks the others' gap tile reservations. A signal stays red if any gap tile between the signal and the guarded block is already reserved by another train, preventing conflicting turnout switching. The turnout aspect of a reserved gap tile is not overridden by another simulation. Gap tiles are also reserved when a train enters a new block during normal movement (not just at signals or simulation start), ensuring consistent protection even without a signal guarding the block boundary.
