...
- **Signal stops**: During occupancy simulation, a train arriving at a main signal (SIGNAL_M3 or SIGNAL_COMBINED) with aspect 0 (red) stops and waits. Signals have an implicit facing direction based on rotation (rot 0 → faces LEFT). Only trains entering from the signal's facing port are blocked; trains approaching from behind ignore the signal. `isSignalBlocking(Tile, int entryPort)` implements this check. When `autoChangeSignal` is enabled, a blocked signal auto-switches to aspect 1 after 2 seconds. Toggling this option immediately affects all running simulations. **Signal reset to red**: when a green signal is passed by the train head (configurable distance, default 5 tiles ahead), it automatically resets to red. This applies to the start signal (set green when simulation starts), auto-change green, and station-dwell green signals.
- **Train length**: Simulated trains have a configurable length (default 3 tiles). The train occupies its head tile (current path position) plus up to 2 trailing cars placed **behind the head along the physical track** (not necessarily on the route path). At start, the trailing cars are placed by walking backward from the first route tile along connected track in the opposite travel direction. During movement, the train advances as a sliding window: the head moves to the next path tile and the tail frees the oldest tile. The length is configurable via `RouteSimulation.setTrainLength()` / `OccupancySimulation.setTrainLength()` (minimum 1).
- **Distant signals (SIGNAL_V)**: Distant signals never stop the train. Each distant signal on a route mirrors the aspect of its linked main signal (via `SignalTile.mainSignalId`), previewing the upcoming aspect: orange "Halt erwarten", green "Frei erwarten", orange+green "Langsamfahrt erwarten". The preview is mapped from the linked main signal's current aspect. Mirroring is applied on simulation start and refreshed every simulation step. The fourth aspect (aspect 3: bottom-right orange + both green lights) is only reachable by manually cycling the signal.
- **Combined signals (SIGNAL_COMBINED)**: A combined signal carries its own main head (top, own aspect: red/green/orange, cycled by clicking like SIGNAL_M3) and a distant plate (bottom) that mirrors its linked main signal. Both fragments are composed at runtime from separate SVGs (head + plate) without stretching. The plate always mirrors the linked main signal's live aspect. The plate aspect range is 3 (orange, green, orange+green) and is shown in the **Tile Info** dialog. Combined signals support the same signal-side override as SIGNAL_M3/SIGNAL_V.
...
| `signal_sm_head_red_left.svg` / `_right` | <img src="switchboard-component/src/main/resources/icons/signals/sbb_l/signal_sm_head_red_left.svg" width="32" height="32"> | SBB combined signal (Kombi-Lichtsignal) main head fragment — red lit, orange+green lamps dim |
| `signal_sm_head_green_left.svg` / `_right` | <img src="switchboard-component/src/main/resources/icons/signals/sbb_l/signal_sm_head_green_left.svg" width="32" height="32"> | SBB combined signal main head fragment — green lit |
| `signal_sm_head_orange_left.svg` / `_right` | <img src="switchboard-component/src/main/resources/icons/signals/sbb_l/signal_sm_head_orange_left.svg" width="32" height="32"> | SBB combined signal main head fragment — orange lit |
| `signal_sm_plate_orange_left.svg` / `_right` | <img src="switchboard-component/src/main/resources/icons/signals/sbb_l/signal_sm_plate_orange_left.svg" width="32" height="32"> | SBB combined signal distant plate fragment — two orange lit ("Halt erwarten"), carries the shared mast |
| `signal_sm_plate_green_left.svg` / `_right` | <img src="switchboard-component/src/main/resources/icons/signals/sbb_l/signal_sm_plate_green_left.svg" width="32" height="32"> | SBB combined signal distant plate fragment — one green lit ("Frei erwarten") |
| `signal_sm_plate_orgreen_left.svg` / `_right` | <img src="switchboard-component/src/main/resources/icons/signals/sbb_l/signal_sm_plate_orgreen_left.svg" width="32" height="32"> | SBB combined signal distant plate fragment — orange+green lit ("Langsamfahrt erwarten") |

Combined signals (SIGNAL_COMBINED) are rendered by composing the main head and distant plate fragments at runtime (see `SwitchboardPanel.drawCombinedSignalTile()`); each fragment keeps its original level aspect ratio with no stretching.
...
| `suggestMainSignalFollowsTrackAroundCurveCorner` | Auto-suggest follows the connected physical track around a diagonal curve corner to find the main signal (uses `switchboard3c.json`) |
...
| `rotateDistantSignalAutoAssignsMainSignalAhead` | Rotating an unlinked SIGNAL_V (Ctrl+R) auto-assigns the main signal ahead and switches to its aspect (uses `switchboard3b.json`) |
| `rotateDistantSignalReassignsNewMainSignalAhead` | Rotating a linked SIGNAL_V replaces the old link with the main signal now ahead and syncs its aspect (uses `switchboard3b.json`) |
...
- The train head follows the route path; trailing cars are placed behind the head along the **physical track** (not necessarily on the route path). At start, cars are placed by walking backward from the first route tile along connected track in the opposite travel direction (using `RouterService.pickBackwardNeighbor()`). During movement, the train advances as a sliding window: the head moves to the next path tile and the tail frees the oldest tile.
...
- **Signal reset after train passes**: When a signal turns green (auto-change, station dwell, or start signal), it automatically resets to red after the train head passes a configurable distance ahead (default 5 tiles, configurable via `RouteSimulation.setSignalResetDistance(int)`).
- **Distant signal sync via mainSignalId**: Distant signals (SIGNAL_V) now sync with their configured linked main signal (`SignalTile.mainSignalId`) instead of scanning forward in the path for the next main signal. This ensures each distant signal only previews its actual linked main signal.
- **Block marker cleanup**: When the train head leaves a block, the block marker's train assignment is cleared after a 5-second delay. A single 1s timer iterates a map of block departure timestamps, clearing each block independently when the delay elapses.
- **Self-driving simulation timer**: `RouteSimulation` now owns its own timer (configurable interval, default 200ms) that drives both `tick()` and block cleanup (every 5th tick). The `SwitchboardPanel` no longer manages a separate `routeTimer`. An `onComplete` callback notifies the panel when the route finishes naturally.
- **Configurable tick interval**: `RouteSimulation.setTickInterval(int ms)` sets the simulation step interval (minimum 50ms, default 200ms).
